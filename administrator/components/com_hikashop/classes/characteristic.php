	...
	public function &getNameboxData($typeConfig, &$fullLoad, $mode, $value, $search, $options) {
		...
		if(isset($typeConfig['params']['value']) && $typeConfig['params']['value']) {
			if((int)$options['url_params']['ID'] > 0) {
				$query = 'SELECT characteristic_value, characteristic_id FROM ' . hikashop_table('characteristic').' WHERE characteristic_parent_id = ' . (int)$options['url_params']['ID'];
			...
			$query = 'SELECT characteristic_value, characteristic_id FROM ' . hikashop_table('characteristic').' WHERE characteristic_parent_id = 0';
			if(!empty($options['vendor']))
				$query .= ' AND characteristic_vendor_id IN (0, '.(int)$options['vendor'].')';
			$this->database->setQuery($query);
			$characteristics = $this->database->loadObjectList('characteristic_id');
		...
		if(!empty($characteristics)) {
			foreach($characteristics as $k => $v) {
				$ret[0][$k] = $v->characteristic_value;
			}
			asort($ret[0]);
		}
