...
class hikashopCurrencyClass extends hikashopClass{
	...
	function getTaxedPrice(&$price, $zone_id, $tax_category_id, $round = 2) {
		...
		if(!$config->get('floating_tax_prices', 0)) {
			...
			$taxedPrice = $this->_round($float_price + $float_price * $tax, $round);
			if(!empty($this->taxRates)) {
				foreach($this->taxRates as $k => $rate) {
					$this->taxRates[$k]->tax_amount = $this->_round($float_price*floatval($rate->tax_rate),$round);
				}
			}
		} else {
			$taxedPrice = (float)$price;
			$price = $this->_round($taxedPrice / (1.00000 + $tax), $round);
			if(!empty($this->taxRates)){
				foreach($this->taxRates as $k => $rate){
					$this->taxRates[$k]->tax_amount = $this->_round($taxedPrice * floatval($rate->tax_rate) / (1.00000 + floatval($rate->tax_rate)), $round);
				}
			}
		}
		return $taxedPrice;
	...
	function selectDiscount(&$product,&$discounts,$zone_id,$parent=null){
		...
		foreach($discounts as $discount){
			...
			if(empty($discount->discount_product_id) && !empty($discount->discount_category_id)){
				if(!is_array($discount->discount_category_id)) $discount->discount_category_id = explode(',',trim($discount->discount_category_id,','));
				if($discount->discount_category_childs){
					static $childs=array();
					$key = implode(',',$discount->discount_category_id);
					if(!isset($childs[$key])){
						$classCategory = hikashop_get('class.category');
						$childs[$key]=$classCategory->getChilds($discount->discount_category_id,true,array(),'',0,999,false,'a.category_id');
			...
			if(isset($price->taxes)) {
				foreach($price->taxes as $tax){
					if(isset($total->taxes[$tax->tax_namekey])){
						$total->taxes[$tax->tax_namekey]->tax_amount += $this->_round($tax->tax_amount,$rounding);
					} else {
